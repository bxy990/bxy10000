...
def gou_wu():
    ...
    class Goods:
        def __init__(self):
            self.cid = input("请输入商品id")

        def choise_goods(self):
            pass

    # 商品多态
    class Good_info(Goods):
        def choise_goods(self):
            if self.cid in shang_pin_info:
                print("添加到购物车")
            return {"cid": cid, "count": count}
    class Good_out(Goods):
        def choise_goods(self):
            if self.cid not in shang_pin_info:
                print("该商品不存在")


    while True:
        item = input("输入1购买，输入2结算")
        if item == "1":
